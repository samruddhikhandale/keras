...
class DatasetCreatorAdapter(DataAdapter):
    ...
    def __init__(self, x, y, steps=None, distribution_strategy=None, **kwargs):
        ...
        if steps is None:
            if not kwargs.get("exact_evaluation"):
                raise ValueError(
                    "When using a "
                    "`tf.keras.utils.experimental.DatasetCreator`, "
                    "`steps_per_epoch`, `validation_steps`, `steps`, or "
                    "`exact_evaluation` argument must be provided in "
...
class DatasetAdapter(DataAdapter):
    ...
    def __init__(self, x, y=None, sample_weights=None, steps=None, **kwargs):
        ...
        self._validate_args(
            y, sample_weights, steps, kwargs.get("exact_evaluation")
        )
    ...
    def _validate_args(self, y, sample_weights, steps, exact_evaluation):
        ...
        if steps is None and not exact_evaluation:
            if _is_distributed_dataset(self._dataset):
                raise ValueError(
...
class DataHandler:
    ...
    def __init__(
        ...
        Arguments:
          ...
          exact_evaluation: See `Model.fit`.
        """
...
class _ClusterCoordinatorExactEvalDataHandler(_ClusterCoordinatorDataHandler):
    def __init__(self, x, y=None, **kwargs):
        logging.info("Initing _ClusterCoordinatorExactEvalDataHandler")
        super().__init__(x=x, **kwargs)
        self._total_shards = kwargs.get("exact_evaluation")
...
def get_data_handler(*args, **kwargs):
    ...
    if getattr(kwargs["model"], "_cluster_coordinator", None):
        if kwargs.get("exact_evaluation"):
